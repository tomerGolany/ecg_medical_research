...
class Inception3(torch.nn.Module):
    ...
    def __init__(self, num_classes=1000, aux_logits=True, transform_input=False,
        ...
        self.maxpool1 = torch.nn.MaxPool1d(kernel_size=3, stride=2)
        self.Conv2d_3b_1x1 = conv_block(64, 80, kernel_size=1)
        self.Conv2d_4a_3x3 = conv_block(80, 192, kernel_size=3)
        self.maxpool2 = torch.nn.MaxPool1d(kernel_size=3, stride=2)
...
class InceptionA(torch.nn.Module):
    ...
    def _forward(self, x):
        ...
        branch_pool = torch.nn.functional.avg_pool1d(x, kernel_size=3, stride=1, padding=1)
...
class InceptionB(torch.nn.Module):
    ...
    def _forward(self, x):
        ...
        branch_pool = torch.nn.functional.max_pool1d(x, kernel_size=3, stride=2)
...
class InceptionAux(torch.nn.Module):
    ...
    def forward(self, x):
        # N x 768 x 17 x 17
        x = torch.nn.functional.avg_pool1d(x, kernel_size=5, stride=3)
        # N x 768 x 5 x 5
        x = self.conv0(x)
        # N x 128 x 5 x 5
        x = self.conv1(x)
...
class BasicConv2d(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, **kwargs):
        ...
        self.bn = torch.nn.BatchNorm1d(out_channels, eps=0.001)
